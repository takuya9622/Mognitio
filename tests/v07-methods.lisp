...
(defparameter *v07-contract-source*
  "type U=struct{s:string;}; type V=enum{Item(string);}; interface Name{function name():string;} type N=Name; implement U against N{let name=function():string{this->s};} implement V against Name{let name=function():string{branch on(this){V::Item(s)=>s}};} let render=function(n:N):string{n->name()};")
(defparameter *v07-same-name-source*
  "type U=struct{}; interface A{function name():string;} interface B{function name():string;} type Alias=A; implement U against Alias{let name=function():string{\"a\"};} implement U against B{let name=function():string{\"b\"};} let a=function(x:A):string{x->name()};let b=function(x:B):string{x->name()};")
...
(deftest v07-methods-and-dispatch
  ...
  (dolist (source
    '("type U=struct{s:string;};let raw=function(u:U):string{u->s};implement U{let name=function():string{raw(this)};let copy=function():U{return this;};let value=function():U{this};} U{s:\"ok\"}->copy()->value()->name()==\"ok\""
      ...
      "type U=struct{};interface A{function f():int;}interface B{function f():int;}implement U against A{let f=function():int{1};}let first=function(u:U):int{u->f()};implement U against B{let f=function():int{2};}first(U{})==1"
      "type U=struct{};interface Empty{}implement U against Empty{}let f=function(e:Empty):bool{true};f(U{})"
      "type U=struct{};interface A{function a():int;}let use=function(x:A):int{x->a()};implement U against A{let a=function():int{7};let extra=function():int{1};}true"))
  ...
  (dolist (source
    '("type U=struct{};interface A{function f():int;}implement U against A{}true"
      "type U=struct{};interface A{function f():int;}implement U against A{let f=function():bool{true};}true"
      "type U=struct{};interface A{function f(x:int):int;}implement U against A{let f=function():int{1};}true"
      ...
      "type U=struct{};interface A{function f():int;}implement U{let f=function():int{1};}implement U against A{let f=function():int{2};}U{}->f()==1"
      "type U=struct{};interface A{function f():int;}interface B{function f(x:int):int;}implement U against A{let f=function():int{1};}implement U against B{let f=function(x:int):int{x};}U{}->f()==1"
      ...
      "interface Step{function step():bool;}type Stop=struct{};implement Stop against Step{let step=function():bool{true};}let apply=function(v:Step):bool{v->step()};type Again=struct{};implement Again against Step{let step=function():bool{apply(Stop{})};}apply(Stop{})"
      "interface A{function f():int;}type U=struct{};implement U{let f=function():int{1};}let use=function(a:A):int{a->f()};use(U{})==1"
  ...
  (dolist (source '("implement int{let f=function():int{1};}true"
                    "interface A{function f():int{1}}true" "interface A{let x=1;}true"

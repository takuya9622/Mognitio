...
(deftest v07-data-and-source-order
  ...
  (dolist (source
    '("type N=int; type S=string; let f=function(n:N,s:S,):int{n+s->length()}; f(2,\"abc\",)==5"
      "type Z=struct{}; type U=struct{n:int;z:Z;}; type A=U; type E=enum{Zero;Pair(U,string,);}; let e=E::Pair(A{z:Z{},n:7,},\"ok\",); branch on(e){E::Zero=>false,E::Pair(u,s,)=>branch when{u->n==7=>s==\"ok\",else=>false,},}"
      "type U=struct{a:int;b:int;}; var n=0; let u=U{b:{n=n+1;n},a:{n=n+1;n}}; u->a*100+u->b*10+n==212"
      "type U=struct{a:int;b:int;}; var n=0; let x=loop{U{b:{break 8;},a:{n=n+1;n}}}; x+n==8"
      "type U=struct{a:int;b:int;}; let f=function():int{U{a:{return 8;},b:1/0}}; f()==8"
      "type U=struct{a:int;}; var u=U{a:1}; let old=u; u=U{a:2}; old->a*10+u->a==12"
...
(deftest v07-branch-contracts
  ...
  (dolist (source
    '("var n=0; let x=branch when{{n=n+1;false}=>1,{n=n+1;true}=>{n=n+10;2},{n=n+100;true}=>1/0,else=>0};x+n==14"
      ...
      "var i=0; let x=loop{i=i+1;branch when{i<3=>{continue;},else=>{break 8;}};};x+i==11"
      "let a=function():int{1};let b=function():int{2};let f=branch when{false=>a,else=>b};f()==2"
  ...
  (dolist (source
    '("type E=enum{A;B;};branch on(E::A){E::A=>void};true"
      ...
      "type E=enum{A(int);};let x=1;branch on(E::A(1)){E::A(x)=>true}"

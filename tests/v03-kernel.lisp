...
(deftest v03-positive-kernel
  (dolist (text
            '("(true)" "let unitPrice = 120; var count = 2; count = count + 1; count * unitPrice == 360"
              "let a = 1 + 2 * 3; let b = (1 + 2) * 3; branch when{(a == 7)=>{ b == 9 },else=>{ false }}"
              ...
              "let n = branch when{(false)=>{ 1 },else=>{ 2 }}; n == 2"
              "let chosen = branch when{(true)=>{ let result = 1; result },else=>{ let result = 2; result }}; chosen == 1"
              "var n = 0; let ok = branch when{(true)=>{ n = 2; true },else=>{ n = 9; false }}; branch when{(ok)=>{ n == 2 },else=>{ false }}"
              "var value = 1; let sum = (branch when{(true)=>{ value = 2; value },else=>{ 0 }}) + value; sum == 4"
              "var value = 1; let sum = value + (branch when{(true)=>{ value = 2; value },else=>{ 0 }}); sum == 3"
              "var value = 8; let n = value / (branch when{(true)=>{ value = 2; value },else=>{ 1 }}); n == 4"
              "var value = 8; let n = value - (branch when{(true)=>{ value = 2; value },else=>{ 1 }}); n == 6"
              "branch when{(false)=>{ 1 / 0 },else=>{ 1 }} == 1"
              "branch when{(true)=>{ 1 },else=>{ 1 % 0 }} == 1"
              "let x = 1; let y = branch when{(true)=>{ var localX = 2; localX = 3; localX },else=>{ 0 }}; branch when{(y == 3)=>{ x == 1 },else=>{ false }}"
              "var ok = false; ok = true; ok"
              "let true1 = 1; let Let = 2; let intValue = 3; let _a2 = 4; true1 + Let + intValue + _a2 == 10"
              "let x = 1; let X = 2; x + X == 3"
              "let first = branch when{(true)=>{ let item = 1; item },else=>{ 0 }}; let item = 2; branch when{(first == 1)=>{ item == 2 },else=>{ false }}"
              "var a = 1; var b = 2; let x = branch when{(true)=>{ a = 3; b = 4; a + b },else=>{ b = 9; 0 }}; branch when{(x == 7)=>{ a * b == 12 },else=>{ false }}"
              "var a = 1; let x = branch when{(branch when{(true)=>{ a = 5; true },else=>{ false }})=>{ a },else=>{ 0 }}; x == 5"
              "var a = 1; let x = branch when{(false)=>{ a = 9; 0 },else=>{ branch when{(true)=>{ a = 2; a },else=>{ 1 }} }}; branch when{(x == 2)=>{ a == 2 },else=>{ false }}"))
...
(deftest v03-rejection-kernel
  ...
  (dolist (text '("let x = x; true" "true1" "truefalse" "x = 1; true"
                  ...
                  "let y = branch when{(true)=>{let x = 1; x},else=>{0}}; x == 1"
                  "let y = branch when{(true)=>{let x = 1; x},else=>{x}}; y == 1"
                  ...
                  "let x = 1; let y = branch when{(false)=>{let z = branch when{(true)=>{var x = false; 0},else=>{0}};z},else=>{0}};true"))
...
(deftest v03-runtime-arithmetic
  ...
  (v03-runtime "branch when{(true)=>{let n = 1 / 0; false},else=>{true}}" "division by zero")
...
(deftest v03-integer-machine-goldens
  ...
  (let* ((ir (native-ir "var x = 1; let y = branch when{(true)=>{x = 2; 3},else=>{4}}; x + y == 5"))
...
(deftest v03-contract-boundaries
  ...
  (dolist (pair '(("var x = 9223372036854775807; " "x + 1")
                  ("var x = -9223372036854775808; " "x - 1")
                  ("var x = 9223372036854775807; " "x * 2")
                  ("var x = -9223372036854775808; " "x / -1")
                  ("var x = -9223372036854775808; " "x % -1")))
  ...
  (dolist (space (list " " (string #\Tab) (string #\Newline) (string #\Return)
    ...
    (v03-positive (format nil "let~An~A=~A-~A9223372036854775808~A;~An~A<~A0"
  ...
  (let* ((text "var x = 0; branch when{(true)=>{1 / x},else=>{2}} == 0")
  ...
  (let* ((ir (native-ir "var a = 10; var b = 20; let x = branch when{(true)=>{a = 30; b = 40; let local = 99; 50},else=>{b = 60; 70}}; a + b == x"))
  ...
  (let* ((ir (native-ir "let unused = -1 + 2 - 3 * 4; true"))
  ...
  (dolist (source '("let unused = 1 / 2; true" "let unused = 1 % 2; true"))

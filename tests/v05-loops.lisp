...
(deftest v05-unconditional-loops
  (dolist (source
    '("loop { break; }; true"
      ...
      "let f = function(n: int): int { var i: int = 0; loop { i = i + 1; branch when{(i == n)=>{return i;}}; } }; f(3) == 3"
      "let unused = function(): int { loop {} }; true"
      "let unused = function(): void { loop { continue; } }; true"
      "var n: int = 0; loop { let f = function(): void { loop {break;}; }; f(); n = n + 1; branch when{(n == 2)=>{break;}}; }; n == 2"
  ...
  (dolist (source
    '("break; true" "continue; true" "loop { break; void }"
      ...
      "let f = function(): int { var n: int = 1; n = loop {}; true }; true"))
...
(deftest v05-conditional-and-valued-loops
  (dolist (source
    '("var n: int = 0; loop while(n < 3){ n = n + 1; }; n == 3"
      ...
      "let f = function(): int {1}; let g = function(): int {2}; (loop {branch when{(false)=>{break f;}}; break g;})() == 2"
      "let f = function(stop: bool): int {let x: int = loop {break branch when{(stop)=>{return 7;},else=>{20}};}; x + 1}; f(true) + f(false) == 28"
  ...
  (dolist (pressure '(0 32 600))
    (v03-positive
     (with-output-to-string (out)
       (write-string "let add = function(a: int,b: int): int {a+b}; let sub = function(a: int,b: int): int {a-b}; let identity = function(value: int): int {value}; let base: int = 100; " out)

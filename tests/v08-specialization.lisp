...
(defparameter *v08-instances-source*
  "let identity=function<T>(x:T):T{x}; let alias=identity; let a:int=identity<int>(42); let b:string=alias<string>(\"ok\"); let c:int=alias<int>(a); branch when{c==42=>b==\"ok\",else=>false}")
(defparameter *v08-nested-source*
  "let outer=function<T>():bool{let leaf=function():Result<T,string>{Result<T,string>::Err(\"x\")};let inner=function():bool{branch on(leaf()){Result<T,string>::Ok(_)=>false,Result<T,string>::Err(e)=>e==\"x\"}};inner()}; outer<int>()==outer<string>()")

(deftest v08-instance-closure-and-isolation
  (dolist (pair (list (cons *v08-instances-source* 3) (cons *v08-nested-source* 7)
                     (cons "let unused=function<T>(x:T):T{let inner=function():bool{true};x};let alias=unused;true" 1)
                     (cons "let identity=function<T>(x:T):T{x};let unused=function():int{identity<int>(42)};true" 3)
                     (cons "let outer=function<T>(x:T):bool{let independent=function():bool{true};independent()};outer<int>(42)==outer<string>(\"ok\")" 4)))
...
(deftest v08-concrete-control-and-data
  (dolist (source
           '("let choose=function<T>(x:T,flag:bool):T{loop{branch when{flag=>{return x;}};break x;}};let a:int=choose<int>(42,true);let b:string=choose<string>(\"ok\",false);branch when{a==42=>b==\"ok\",else=>false}"
             "let change=function<T>(x:T,y:T):T{var value:T=x;value=y;value};change<int>(1,42)==42"
             "let choose=function<T>(x:T):T{let a=function(y:T):T{y};let b=function(z:T):T{z};(branch when{true=>a,else=>b})(x)};choose<int>(42)==42"
             "type Box<T> =struct{value:T;};let identity=function<T>(x:T):T{x};type U=struct{box:Box<int>;};interface I{let get=function():int;}implement U against I{let get=function():int{identity<int>(this->box->value)};}let use=function(i:I):int{i->get()};use(U{box:Box<int>{value:42}})==42"
             "let wrap=function<T,E>(value:T):Result<T,E>{Result<T,E>::Ok(value)};let x:Result<void,string> =wrap<void,string>(void);branch on(x){Result<void,string>::Ok(v)=>{v;true},Result<void,string>::Err(_)=>false}"
             "let identity=function<T>(x:T):T{x};branch when{true=>identity<int>(42)==42,else=>identity<string>(\"ok\")==\"ok\"}"))
    (v07-accept source))
  (let ((source "type Box<T> =struct{value:T;};let make=function<T>(x:T):Box<T>{Box<T>{value:x}};let keep:Box<Result<string,int>> =make<Result<string,int>>(Result<string,int>::Ok(\"keep\"+\"!\"));var i:int=0;loop while(i<2000){let dead:Box<string> =make<string>(\"dead\"+\"!\");i=i+1;};branch on(keep->value){Result<string,int>::Ok(s)=>s==\"keep!\",Result<string,int>::Err(_)=>false}"))
    (v07-accept source))

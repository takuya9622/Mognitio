...
(deftest v06-generated-caller-handwritten-text-callee
  (let ((source "let probe:function(string,void,int,bool):string=function(s:string,v:void,n:int,b:bool):string{s}; let r: string=probe(\"a\",void,7,true); let z: string=probe(r,void,7,true); true")
        (callee (append
...
(deftest v06-handwritten-caller-generated-text-callee
  (let* ((source "let probe:function(string,void,int,bool):string=function(s:string,v:void,n:int,b:bool):string{s}; let unused: string=\"a\"; true")
         (caller (append
...
(deftest v06-native-helper-publication-and-loop
  (let* ((source "let identity:function(string):string=function(s:string):string{s}; var keep: string=\"a\"; var i: int=0; loop while(i<100){let temp: string=keep+\"b\"; let n: int=temp->length(); keep=keep->slice(0,1); keep=identity(keep); i=i+1;}; true")
...
(deftest v06-native-context-pagesize
  (multiple-value-bind (out err status)
    ...
    (let ((callee (append
      ...
      (multiple-value-bind (out err code)
          (v05-cross-abi "let probe:function():bool=function():bool{true}; probe()" 1 callee)
        (same 0 code) (same (format nil "true~%") out) (same "" err)))))

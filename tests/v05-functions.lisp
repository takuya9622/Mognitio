...
(deftest v05-function-values-and-scope
  (dolist (source
    '("let f = function(x: int): int { x + 1 }; let alias = ((f)); (alias)(2) == 3"
      ...
      "let f = function(x: int): int { x + 1 }; let g = function(x: int): int { x - 1 }; (if(false){f}else{g})(3) == 2"
      "let f = function(): int { 7 }; let alias = f; let g = function(): int { alias() }; g() == 7"
      "let f = function(): int { let g = function(x: int): int { x + 1 }; g(2) }; f() == 3"
      "let a = function(): int { 10 }; let b = function(): int { 20 }; let choose = if(true){a}else{b}; choose() == 10"
      "let f = function(g: int): int { g }; let g = function(): int { 1 }; f(g()) == 1"
      "let add = function(a: int, b: int): int { a + b }; let sub = function(a: int, b: int): int { a - b }; var flag = true; (if(flag){add}else{sub})(10, if(true){flag = false; 3}else{0}) == 13"))
  ...
  (dolist (source
    '("let x = 1; let f = function(): int { x }; true"
      ...
      "let f = function(): int { 1 }; let selected = if(true){f}else{f}; let g = function(): int { selected() }; true"
      ...
      "let f = function(): int { 1 }; let g = function(): bool { true }; (if(true){f}else{g})()"))
    (v03-reject source "semantic"))
  (v03-reject "function named(): int { 1 } true" "parse"))

(deftest v05-core-function-candidates
  (let* ((module (native-ir "let f = function(): int { 1 }; let g = function(): int { 2 }; (if(true){f}else{g})() == 1"))
